...
public class RDBMSdtd2RDBMSemofTransformer extends AbstractWorkflowComponent2 {
	private String inputSlot;
	private String outputSlot;
	
	private final Map<String, String> config;
	
	public RDBMSdtd2RDBMSemofTransformer() {
	...
	protected void invokeInternal(WorkflowContext ctx, ProgressMonitor m,
		...
		if (inputSlotContent == null) {
			...
			for (Object element : (Collection<?>) inputSlotContent) {
				if (element instanceof /*rdbms.RDBMS*/EObject) {
					EObject inputModel = (EObject) element;
					EcoreUtil.resolveAll(inputModel.eResource());
					
					// execute the transformation
					/*rdbmsMM.dummy*/EObject transformedModel = new RDBMSdtd2RDBMSemof().transform(inputModel, config);
		...
		} else if (inputSlotContent instanceof /*rdbms.RDBMSType*/EObject) {
			
			/*rdbms.RDBMSType*/EObject inputModel = (/*rdbms.RDBMSType*/EObject) inputSlotContent;
			EcoreUtil.resolveAll(inputModel.eResource());
